...
const CheckoutPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    pincode: "",
    altPhone: "",
    primaryPhone: "" // Added to display in read-only section
  });
  ...
  // 1. Fetch Profile Data & Handle Mounting
  useEffect(() => {
    setMounted(true);
      ...
      <header className="border-b border-gray-100 py-6">
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2 text-green-600">
            <Lock size={16} />
            <span className="text-xs font-bold uppercase tracking-widest text-green-600">Secure Checkout</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 lg:py-16">
        <div className="flex flex-col lg:flex-row gap-16 items-start">
              ...
              <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
                ...
                <section className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <input type="text" placeholder="City" value={formData.city} className="w-full p-4 border border-gray-200 rounded-xl text-sm font-semibold focus:border-black outline-none" onChange={(e) => setFormData({...formData, city: e.target.value})} />
                    <input type="text" placeholder="State" value={formData.state} className="w-full p-4 border border-gray-200 rounded-xl text-sm font-semibold focus:border-black outline-none" onChange={(e) => setFormData({...formData, state: e.target.value})} />
                    <input type="text" placeholder="PIN" value={formData.pincode} className="w-full p-4 border border-gray-200 rounded-xl text-sm font-semibold focus:border-black outline-none" onChange={(e) => setFormData({...formData, pincode: e.target.value})} />
                  </div>
                </section>
          ...
          <aside className="w-full lg:w-[40%] lg:sticky lg:top-24 bg-gray-50 border border-gray-100 rounded-[2.5rem] p-8 order-1 lg:order-2">
            ...
            <div className="mt-8 p-4 bg-white rounded-2xl border border-gray-100 flex items-start gap-3">
              <ShieldCheck className="text-green-600 shrink-0" size={20} />
              <p className="text-[10px] font-bold leading-relaxed text-gray-500 uppercase tracking-wider">
                Order includes <span className="text-black">22K Gold Foil Certificate</span> and Teak Wood framing.
              </p>
            </div>
          </aside>
